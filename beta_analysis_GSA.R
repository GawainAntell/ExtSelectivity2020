...
###################################################
# multipanel plot of beta as a function of n sites
...
x_mx <- max(ssbysed$sites)*1.1
y_mx <- max(ssbysed$beta, na.rm=TRUE)*1.1
y_mn <- 0 # min(ssbysed$beta, na.rm=TRUE) - .1

p <- list()
for (b in bins){
  bin_bool <- ssbysed$stage==b
  b_dat <- ssbysed[bin_bool,]
  b_dat$mu <- as.factor(b_dat$mu)
  stage_bool <- stage_dat$stage==b
  site_max <- stage_dat$gcell[stage_bool]
  mst_max <- stage_dat$dispersion[stage_bool]
  titl <- b

  p[[b]] <- 
    ggplot(data=b_dat) + theme_bw() +
    ggtitle(label=titl) +
    scale_x_continuous(limits=c(0, x_mx), expand=c(0,0), name='') +
    scale_y_continuous(limits=c(y_mn, y_mx), expand=c(0,0), name='') +
    geom_point(aes(x=sites, y=beta, color=mu), 
               position=position_jitterdodge(dodge.width=.2)) +
    geom_vline(xintercept = site_max, linetype='dashed') +
    theme(legend.position = 'none')
}

wLgnd <- ggplot(data=b_dat) + theme_bw() +    
  geom_point(aes(x=sites, y=beta, color=mu)) 
lgnd <- get_legend(wLgnd)

# print order of sediment area - order of plot arrangement
  # stage_dat$stage[order(stage_dat$dispersion)]

figNm <- 'Results/panel_beta_plot'
if (h0){ figNm <- paste0(figNm, '_null')}
if (static){ figNm <- paste0(figNm, '_static')}

pdf(paste0(figNm,'_MSTorder_bySed.pdf'), width=7.5, height=9)
grid.arrange(p[['Lopingian']], p[['Guadalupian']], p[['Meramecian']], 
             p[['Leonardian']], p[['Chesterian']], p[['Kinderhookian']],
             p[['Wolfcampian']], p[['Osagean']], p[['Virgilian']], 
             p[['Morrowan']], p[['Missourian']], p[['Desmoinesian']], 
             p[['Atokan']], lgnd, nrow=4)
dev.off()
